...
void Boundaries_test() {
    ...
    if ((CheckValues(boundaries[0], true_boundaries[0])) || (CheckValues(boundaries[1], true_boundaries[1]))) {
        cout << "Error occured in GetVolumeBoundaries method when evaluating in Volume #0 for initial position  (" << position.X() << ", "
             << position.Y() << ", " << position.Z() << ") and direction (" << direction.X() << ", "
    ...
    if ((CheckValues(field_boundaries[0], true_field_boundaries[0])) || (CheckValues(field_boundaries[1], true_field_boundaries[1]))) {
        cout << "Error occured in GetFieldBoundaries method when evaluating in Volume #0 for initial position  (" << position.X() << ", "
             << position.Y() << ", " << position.Z() << ") and direction (" << direction.X() << ", "
             << direction.Y() << ", " << direction.Z() << ")" << endl;
        return 5;
    }
    ...
    if ((CheckValues(boundaries[0], true_boundaries[0])) || (CheckValues(boundaries[1], true_boundaries[1]))) {
        cout << "Error occured in GetVolumeBoundaries method when evaluating in Volume #1 for initial position  (" << position.X() << ", "
             << position.Y() << ", " << position.Z() << ") and direction (" << direction.X() << ", "
             << direction.Y() << ", " << direction.Z() << ")" << endl;
        return 5;
    }
    ...
    if ((CheckValues(field_boundaries[0], true_field_boundaries[0])) || (CheckValues(field_boundaries[1], true_field_boundaries[1]))) {
        cout << "Error occured in GetFieldBoundaries method when evaluating in Volume #1 for initial position  (" << position.X() << ", "
             << position.Y() << ", " << position.Z() << ") and direction (" << direction.X() << ", "
             << direction.Y() << ", " << direction.Z() << ")" << endl;
        return 5;
    }
...
Int_t CheckValues(TVector3 real_value, TVector3 true_value) {
    if ((fabs(true_value.X() - real_value.X()) <= 0.0001 * std::max(fabs(true_value.X()), fabs(real_value.X()))) &&
        (fabs(true_value.Y() - real_value.Y()) <= 0.0001 * std::max(fabs(true_value.Y()), fabs(real_value.Y()))) &&
        (fabs(true_value.Z() - real_value.Z()) <= 0.0001 * std::max(fabs(true_value.Z()), fabs(real_value.Z()))))
        return 0;
    else return 5;
}
