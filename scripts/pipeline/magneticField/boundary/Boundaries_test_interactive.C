...
void PrintVolumeBoundaries(Int_t volume_ID, TVector3 position, TVector3 direction,
                           std::vector<TVector3> boundaries) {
    if (volume_ID == 0) {
        cout << "Initial position : (" << position.X() << ", " << position.Y() << ", " << position.Z() << ")"
             << endl;
    ...
    for (int i = 0; i < boundaries.size(); i++)
        cout << "boundaries[" << i << "] = (" << boundaries[i].X() << ", " << boundaries[i].Y() << ", "
             << boundaries[i].Z() << ")" << endl;
    cout << endl << endl;
...
void PrintFieldBoundaries(std::vector<TVector3> fieldboundaries) {
    ...
    for (int i = 0; i < fieldboundaries.size(); i++)
        cout << "fieldboundaries[" << i << "] = (" << fieldboundaries[i].X() << ", " << fieldboundaries[i].Y()
             << ", " << fieldboundaries[i].Z() << ")" << endl;
    cout << "---------------------------------" << endl;
