...
void GetMagneticField_test(){
  ...
  myfile.open ("/home/kreso/RestAxionLib/scripts/GetMagneticField_test/GetMagneticField_test_output.txt");
  myfile << std::setprecision(1) << std::fixed;
  cout << std::setprecision(1) << std::fixed;
  myField = new TRestAxionMagneticField("/home/kreso/RestAxionLib/scripts/GetMagneticField_test/my_metadata.rml","bField");
...
void PrintPoint(TVector3 point){
  ...
  cout << "x = " << point.X() + offset.X() << "  y = " << point.Y() + offset.Y() << "  z = " << point.Z() + offset.Z() << endl;
  cout << "Bx = " << B.X() << "  By = " << B.Y() << "  Bz = " << B.Z() << "   True values: " << "TBx = " << true_value.X() << "  TBy = " << true_value.Y() << "  TBz = " << true_value.Z() << endl;
  cout << "Difference = (" << true_value.X()-B.X() << ", " << true_value.Y()-B.Y() << ", " << true_value.Z()-B.Z() << ")" << endl << endl; 
  
  myfile << "x = " << point.X() + offset.X() << "  y = " << point.Y() + offset.Y() << "  z = " << point.Z() + offset.Z() << "\n";
  myfile << "Bx = " << B.X() << "  By = " << B.Y() << "  Bz = " << B.Z() << "   True values: " << "TBx = " << true_value.X() << "  TBy = " << true_value.Y() << "  TBz = " << true_value.Z() << "\n";
  myfile << "Difference = (" << true_value.X()-B.X() << ", " << true_value.Y()-B.Y() << ", " << true_value.Z()-B.Z() << ")" << "\n\n";
