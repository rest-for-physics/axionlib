...
void TRestAxionSolarHiddenPhotonFlux::LoadWidthTable() {
    if (fFluxDataFile == "") {
        RESTError << "TRestAxionSolarHiddenPhotonFlux::LoadWidthTable. No width table was defined"
                  << RESTendl;
        return;
    }
...
///
TH1D* TRestAxionSolarHiddenPhotonFlux::GetContinuumSpectrum() {
...
///
TH1D* TRestAxionSolarHiddenPhotonFlux::GetTotalSpectrum() {
...
std::pair<Double_t, Double_t> TRestAxionSolarHiddenPhotonFlux::GetRandomEnergyAndRadius(TVector2 eRange) {
    ...
    for (unsigned int r = 0; r < fFluxTableIntegrals.size(); r++) {
        if (rnd < fFluxTableIntegrals[r]) {
            ...
            Double_t radius = ((Double_t)r + fRandom->Rndm()) * 0.001;
...
///////////////////////////////////////////////
/// \brief It prints on screen the flux table that has been read from file
///
