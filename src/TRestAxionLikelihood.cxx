...
void TRestAxionLikelihood::GenerateMonteCarlo( )
{
	...
	else if ( fTExpPerStep == -2 ) // TODO KSVZ exclusion
	{
		...
		Double_t shareTime = (fExposureTimePerStep[4] + fExposureTimePerStep[5] + fExposureTimePerStep[6])/7.;
		cout << "Share time : " << shareTime/12. << endl;
		for( int n = 0; n < 7; n++ )
			fExposureTimePerStep[n] = shareTime;
	...
	*/
	...
	for( int n = 0; n < fNSteps; n++ )
	{
		...
		debug << "Step : " << n << " measured : " << counts << " :: " << fMeasuredCountsPerStep.back() << " counts" << endl;
...
void TRestAxionLikelihood::LikelihoodTest( string fname )
{
	...
	for( Double_t m = 0.008; m < 10; m = m * 1.04 )
	{
		...
		cout << "-------------------------------_" << endl;
		for( Double_t g4 = 1.e-10; g4 < 1.e10; g4 = g4 * 1.02 ) 
		{
	...
	//	GetChar();
		...
		for( Double_t g4 = 1.e-10; g4 < 1.e10; g4 = g4 * 1.02 ) 
		{
			...
			if( int95/integral > 0.95 ) { gLimit = g4; cout << "gLimit : " << g4 << endl; break; }
...
Double_t TRestAxionLikelihood::LogLikelihood( Double_t ma, Double_t g10_4, Double_t Nmeas, Double_t rho, Double_t tExp )
{
	...
	if( Nmeas > 0 )
		lhood += TMath::Exp( -mu + Nmeas ) * pow( mu/Nmeas, Nmeas );
...
void TRestAxionLikelihood::InitFromConfigFile()
{
	...
	fTExpPerStep = StringToDouble( GetParameter( "expTimePerStep", "54" ) ); // In hours (gas phase, time at each step)
