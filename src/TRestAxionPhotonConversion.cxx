...
Double_t TRestAxionPhotonConversion::GammaTransmissionProbability( Double_t Ea, TVectorD B, Double_t ma, Double_t Lcoh)
{
	// If we provide the values as argument we update the class members,
    	// If not, we use the existing values in the class member
    	if (ma == -1)
	...
	for(Int_t i=0;i<=N;i++) {
		b=B[i];	
		Bx[i]=b*TMath::Exp((GammaL/2.0)*(Double_t(i)/Double_t(N)-1.0))*TMath::Cos(-cohLength*PhMeterIneV*(Double_t(i)/Double_t(N))*q);
		By[i]=b*TMath::Exp((GammaL/2.0)*(Double_t(i)/Double_t(N)-1.0))*TMath::Sin(-cohLength*PhMeterIneV*(Double_t(i)/Double_t(N))*q);		      ind[i]=cohLength*(Double_t(i)/Double_t(N));}
