...
int TransmissionSensitivity( std::string rmlFile, std::string section, Double_t years, std::string outputFile = "/tmp/IAXOlimit.txt")
{
	...
	for( double mass = fromMass; mass <= toMass; mass *= step )
	{
		...
		Double_t gag_vacuum = TMath::Power(TMath::Log(20)/helioscopeVacuum.GetSignalRate(mass, fromEnergy, toEnergy)/tExposure, 0.25) * 1.e-10;
		Double_t gag_gas = TMath::Power(TMath::Log(20)/helioscopeGas.GetSignalRate(mass, fromEnergy, toEnergy)/tExposure, 0.25) * 1.e-10;
