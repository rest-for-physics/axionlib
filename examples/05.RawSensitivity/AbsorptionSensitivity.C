...
int AbsorptionSensitivity(std::string rmlFile, std::string section, Double_t years,
                          std::string outputFile = "/tmp/CMSlimit.txt") {
    ...
    for (double mass = fromMass; mass <= phMass; mass *= step) {
        Double_t gag =
            TMath::Power(TMath::Log(20) / helioscope.GetSignalRate(mass, fromEnergy, toEnergy) / tExposure,
                         0.25) *
            1.e-10;
        fprintf(f, "%lf\t%e\n", mass, gag);
    }
